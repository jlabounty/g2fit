...
Modified for CENPA-Rocks cluster by Josh LaBounty (July 2020)
...
def _parse_qsub_job_id(qsub_out):
    ...
    """
    logger.debug("QSUB ID: "+str(qsub_out.split()[2]))
    return int(str(qsub_out.split()[2]))
...
class SGEJobTask(luigi.Task):
    ...
    shared_tmp_dir = luigi.Parameter(default='/home/labounty/luigi', significant=False)
    parallel_env = luigi.Parameter(default='smp', significant=False)
    ...
    def _fetch_task_failures(self):
        ...
        with open(self.errfile, "r") as f:
            errors = f.readlines()
            errors = [i for i in errors if not ("WARNING" in i)]
        if errors == []:
            return errors
    ...
    def _init_local(self):
        ...
        logger.info("Dumping pickled class")
        ...
        if not self.no_tarball:
            ...
            logger.info("Tarballing dependencies")
    ...
    def _smart_dump(self, data, formatter,queue_dir="/home/labounty/stupid_queue/"):
        '''
            "Smart" implementation of output().dump which takes into consideration the queue on rocks
        '''
        print("Initiating smart dump using queue:", queue_dir)
    ...
    def _dump(self, out_dir=''):
        ...
        # logger.info("Dumping pickled class2")
        with self.no_unpicklable_properties():
            # logger.info("Dumping pickled class3")
            self.job_file = os.path.join(out_dir, 'job-instance.pickle')
            if self.__module__ == '__main__':
                # logger.info("Dumping pickled class4")
                d = pickle.dumps(self)
                # logger.info("Dumping pickled class4.1")
                module_name = os.path.basename(sys.argv[0]).rsplit('.', 1)[0]
                # logger.info("Dumping pickled class4.2 "+str(module_name))
                d = d.replace(b'c__main__', str.encode("c" + module_name))
                # logger.info("Dumping pickled class4.5")
                print(d, type(d))
                with open(self.job_file, "wb") as f:
                    f.write(d)
                # logger.info("Dumping pickled class5")
            else:
                with open(self.job_file, "wb") as f:
                    pickle.dump(self, f)
                    # logger.info("Dumping pickled class6")
    ...
    def _run_job(self):
        ...
        logger.debug("Output: "+output.decode())
        self.job_id = _parse_qsub_job_id(str(output.decode()))
    ...
    def _track_job(self):
        while True:
            ...
            logger.debug("Job status: "+qstat_out)
